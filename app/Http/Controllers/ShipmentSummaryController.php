...
class ShipmentSummaryController extends Controller
{
    public function index(Request $request)
    {
        ...
        $perPage = $request->input('per_page', 1000); // default 10
    ...
    public function exportShipmentSummary(Request $request)
    {
        $activePeriodId = session('active_period_id');
        ...
        $shipments = Shipment::with(['termin', 'vessel', 'spk', 'fuel'])
        ...
        $period = Periode::find($activePeriodId);

        $pdf = Pdf::loadView('exports.shipment-summary-pdf', compact('shipments', 'period'))
            ->setPaper('a4', 'landscape');

        return $pdf->download('shipment_summary_report.pdf');
    }
}
